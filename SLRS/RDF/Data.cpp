...
bool Data::loadFromStringWhitespace(std::string input, std::string header_line) {
	...
	if (column > num_cols) {
		throw std::runtime_error("Too many columns in a row.");
	}
	else if (column < num_cols) {
		throw std::runtime_error("Too few columns in a row. Are all values numeric?");
	}
	++row;
